...
import fs from "fs"
import { supabaseAdmin } from "../_lib/supabaseAdmin.js" // ✅ samakan dengan inspections
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!inspection_id) return res.status(400).json({ error: "inspection_id required" })
    if (!Number.isFinite(max_height_m)) return res.status(400).json({ error: "max_height_m must be number" })
    ...
    // Insert to DB (tabel kamu: inspection_measures)
    const { data, error } = await supabaseAdmin
      .from("inspection_measures")
      .insert({
        inspection_id,
    ...
    // Optional: update summary di inspections (jangan bikin crash kalau gagal)
    ...
    // bersihin temp file formidable (optional tapi rapi)
    try {
