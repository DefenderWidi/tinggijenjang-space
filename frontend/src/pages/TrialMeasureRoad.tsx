...
export default function MeasureRoad() {
  // ======= Refs =======
  ...
  const [refMeterStr, setRefMeterStr] = useState("")
  ...
  const refSelected = refName.trim() !== "" && refMeter > 0
  ...
  function drawLine(
    ...
    ctx.save()
    ctx.strokeStyle = tone === "ref" ? "rgba(22,163,74,.55)" : "rgba(37,99,235,.55)"
    ctx.lineWidth = 1.2
    ctx.setLineDash([4, 4])
    ctx.beginPath()
  ...
  function computeStats() {
    ...
    const linesOkCount = meters.length
  ...
  async function canvasToFile(canvas: HTMLCanvasElement) {
    ...
    return new File([blob], `road-measurement-${Date.now()}.png`, { type: "image/png" })
  }
  ...
  async function uploadMeasure(id: string) {
    ...
    fd.append("ref_name", refName.trim())
  ...
  function exportImage() {
    ...
    a.download = "Road_Measurement.png"
    a.click()
  }
    ...
    <AppLayout>
      ...
      <div className="mb-2">
        <div className="text-2xl font-extrabold tracking-tight text-buma-text">
          Workspace Pengukuran Jalan (Mode Ujicoba)
        </div>
      </div>
        ...
        <aside className="relative overflow-hidden rounded-2xl border border-buma-border bg-white shadow-soft">
            ...
            <div className="grid gap-3">
              ...
              <div className="grid gap-1">
                ...
                <input
                  ...
                  placeholder="Contoh: Tanggul Selatan / Windrow A"
                  className={`w-full rounded-xl border px-3 py-2 text-sm outline-none transition ${
                    formError && !areaId
                      ? "border-red-500"
                      : "border-buma-border focus:border-buma-green/60"
              ...
              {formError && !isFormValid && (
                <div className="mt-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600 animate-pulse">
          ...
          <div className="border-t border-buma-border p-4">
            <SectionTitle
              ...
              desc="Masukkan referensi custom, lalu klik 2 titik pada objek acuan."
            />

            <div className="grid gap-3">
              <div className="grid gap-1">
                <label className="text-[11px] font-semibold text-buma-muted">
                  Nama Referensi <span className="text-buma-orange">*</span>
                </label>
                <input
                  value={refName}
                  onChange={(e) => {
                  ...
                  placeholder="Contoh: Windrow / Tanggul / Patok"
                  className={`w-full rounded-xl border px-3 py-2 text-sm outline-none transition ${
                    !refSelected && calError && !refName.trim()
                      ? "border-red-500"
                      : "border-buma-border focus:border-buma-green/60"
                  } bg-white`}
                />
              ...
              <div className="grid gap-1">
                <label className="text-[11px] font-semibold text-buma-muted">
                  Tinggi Referensi (meter) <span className="text-buma-orange">*</span>
                </label>
                <input
                  type="number"
                  min="0"
                  ...
                  className={`w-full rounded-xl border px-3 py-2 text-sm outline-none transition ${
                    !refSelected && calError && refMeter <= 0
                      ? "border-red-500"
                      : "border-buma-border focus:border-buma-green/60"
                  } bg-white`}
                />
              ...
              <button
                ...
                title={!refSelected ? "Isi nama dan tinggi referensi dulu" : "Set 2 titik referensi"}
              >
                Set Point
              </button>

              {calError && !refSelected && (
                <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600 animate-pulse">
                  ⚠ Isi nama referensi dan tinggi referensi sebelum melakukan kalibrasi.
                </div>
              )}
            </div>
          </div>
              ...
              {measureError && (!refSelected || !pixelPerMeter) && (
                <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600 animate-pulse">
                  {!refSelected ? (
                    ...
                    <>⚠ Lakukan <b>kalibrasi</b> dulu lalu pilih 2 titik referensi.</>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="relative">
            ...
            <div className="relative p-4">
              <div className="relative w-full overflow-hidden rounded-2xl border border-buma-border bg-gradient-to-br from-white to-[#EEF6F2] p-3">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative inline-grid grid-cols-2 rounded-xl border border-buma-border bg-white p-[3px] shadow-soft">
                      <div
                      ...
                      <button
                        ...
                        className={`relative z-10 flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-extrabold transition ${
                          mode === "ukur"
                            ...
                            : "text-buma-text hover:bg-black/5"
                        } ${!pixelPerMeter ? "cursor-not-allowed opacity-50 hover:bg-transparent" : ""}`}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                ...
                <div className="mt-3 flex items-center gap-2 overflow-x-auto whitespace-nowrap pb-1 text-xs sm:flex-wrap sm:overflow-visible sm:whitespace-normal">
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-buma-blue/20 bg-buma-blue/10">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
                      height="20"
            ...
            <div className="sticky bottom-0 w-full border-t border-buma-border bg-white/95 backdrop-blur">
              <div className="flex flex-wrap items-center justify-end gap-2 px-4 py-3">
                <button
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#15803D]/20 bg-buma-blue/5 px-4 py-2.5 text-sm font-extrabold text-buma-blue shadow-soft transition-all duration-150 hover:border-buma-blue/40 hover:bg-buma-blue/15 active:scale-95 disabled:opacity-40"
                  type="button"
                  disabled={!imgSrc}
                  onClick={exportImage}
              ...
              <button
  className="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-400 px-4 py-2.5 text-sm font-extrabold text-white shadow-soft cursor-not-allowed"
  type="button"
  disabled
  title="Mode trial - submit dinonaktifkan"
>
  🚧 Trial Mode
</button>
              </div>
